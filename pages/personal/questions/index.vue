        ...
        <div class="container">
            ...
            <div class="fs-questions__content">
                <section class="fs-questions__filter">
                    <div
                        class="fs-questions__filter-row fs-questions__filter-row--email"
                    >
                        <input
                ...
                <section class="fs-questions__wrapper" v-if="!loading">
                    <div class="fs-questions__counter">
                        {{ getCounter }} {{ text }}
                    </div>
                    <template v-for="data in getQuestions" :key="data.id">
                        <div
                            class="fs-questions__item"
                            :class="{
                                'fs-questions__item--bg': data.status === 1,
                            }"
                        >
                            <FsAdmin
                                class="fs-questions__item-icon"
                                v-if="data.category_id === 2"
                            />
                            <FsFood
                                class="fs-questions__item-icon"
                            ...
                            <div
                                class="fs-questions__item-answer"
                                v-if="data.status === 1"
                            >
                                <p><span>Ответ:</span></p>
                                <p>{{ data.response }}</p>
...
export default {
    ...
    computed: {
        ...
        ...mapState(useUserStore, ['isAuthenticated', 'getUser']),
    },
    watch: {
        path() {
            this.loadData();
